...
`tools/list` at `https://gateway.pipeworx.io/cactus/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/cactus_resolve \
  -H 'Content-Type: application/json' \
  -d '{"identifier":"aspirin","representation":"smiles"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/cactus_resolve`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
